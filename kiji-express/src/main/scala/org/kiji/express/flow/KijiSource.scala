...
sealed class KijiTypedSource[+T](
          ...
          override val rowRangeSpec: RowRangeSpec,
          override val rowFilterSpec: RowFilterSpec
                                  )
  ...
  def this(tableAddress: String,
           timeRange: TimeRangeSpec,
           timestampField: Option[Symbol],
           inputColumns: List[ColumnInputSpec] = List(),
           outputColumns: List[ColumnOutputSpec] = List(),
           rowRangeSpec: RowRangeSpec = RowRangeSpec.All,
           rowFilterSpec: RowFilterSpec = RowFilterSpec.NoFilter
            ) = {
    this(tableAddress,
      timeRange,
      timestampField,
      inputColumns.map { column => (Symbol(column.columnName.getName), column)}.toMap,
      outputColumns.map { column => (Symbol(column.columnName.getName), column)}.toMap,
      rowRangeSpec,
      rowFilterSpec
    )
  }
